...
//since each block performs the summation for one run.
{
	...
	int partsum_evals = 0;
    int* pEvals_of_new_entities = cData.pMem_evals_of_new_entities + hipBlockIdx_x * cData.dockpars.pop_size;
  	for (int entity_counter = hipThreadIdx_x;
	     entity_counter < cData.dockpars.pop_size;
	     entity_counter += hipBlockDim_x) 
    {
    ...
    if (hipThreadIdx_x == 0)
    {
        cData.pMem_gpu_evals_of_runs[hipBlockIdx_x] += sSum_evals;
    }
}
