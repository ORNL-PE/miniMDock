...
)
{
	...
	// Derived from autodockdev/maps.py
	for (uint32_t atom_id = hipThreadIdx_x;
	...
	for (int gene_cnt = hipThreadIdx_x;
	...
	// ================================================
	for (uint32_t rotation_counter = hipThreadIdx_x;
	              rotation_counter < cData.dockpars.rotbondlist_length;
	              rotation_counter+=hipBlockDim_x)
	{
...
/*		x10 = cube [idx_100] - cube [idx_000]; // z = 0
	...
	// ================================================
        ...
        if (hipThreadIdx_x==0) for (uint32_t contributor_counter = 0; contributor_counter < cData.dockpars.num_of_intraE_contributors; contributor_counter+= 1) {
#else 
	for (uint32_t contributor_counter = hipThreadIdx_x;
	              contributor_counter < cData.dockpars.num_of_intraE_contributors;
	              contributor_counter+= hipBlockDim_x) {
#endif
...
//								0.0771605f * atomic_distance * desolv_energy;
    ...
    torque_rot.y = 0.0f;
    torque_rot.z = 0.0f;
	for (uint32_t atom_cnt = hipThreadIdx_x;
		  atom_cnt < cData.dockpars.num_of_atoms;
		  atom_cnt+= hipBlockDim_x) {
		float3 r;
    ...
    float gy = 0.0f;
    float gz = 0.0f;
    for (uint32_t idx = hipThreadIdx_x; idx < cData.dockpars.num_of_atoms; idx += hipBlockDim_x)
    {
        gx += gradient[idx].x;
        gy += gradient[idx].y;
        gz += gradient[idx].z;
...
#ifdef REPRO
	// Simplest way to ensure random order of atomic addition doesn't make answers irreproducible: use only 1 thread
	if (hipThreadIdx_x==0) for (uint32_t idx = 0; idx < num_torsion_genes * cData.dockpars.num_of_atoms; idx += 1) {
#else
	for (uint32_t idx = hipThreadIdx_x; idx < num_torsion_genes * cData.dockpars.num_of_atoms; idx += hipBlockDim_x) {
#endif
	...
	for (uint32_t gene_cnt = hipThreadIdx_x+3; // Only for gene_cnt > 2 means start gene_cnt at 3
		  gene_cnt < cData.dockpars.num_of_genes;
		  gene_cnt+= hipBlockDim_x) {
