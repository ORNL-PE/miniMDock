...
)
{
	...
	float  energy = 0.0f;
	int    run_id = hipBlockIdx_x / cData.dockpars.pop_size;
    float* pGenotype = pMem_conformations_current + hipBlockIdx_x * GENOTYPE_LENGTH_IN_GLOBMEM;
	...
	if (hipThreadIdx_x == 0) 
    {
		pMem_energies_current[hipBlockIdx_x] = energy;
...
{
    hipLaunchKernelGGL(gpu_calc_initpop_kernel, dim3(blocks), dim3(threadsPerBlock), 0, 0, pConformations_current, pEnergies_current);
