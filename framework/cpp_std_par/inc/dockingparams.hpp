...
// Docking parameters for NVIDIA HPC SDK implementation
struct DockingParams
{
	...
	unsigned int    g3;
	float           grid_spacing;
	vector<float*> fgrids_write; // View for the copy to device
	vector<const float*> fgrids; // View for the kernels (random access)
        int             rotbondlist_length;
        float           coeff_elec;
        float           coeff_desolv;
        vector<int*> evals_of_new_entities;
        vector<unsigned int*> prng_states;
        int             pop_size;
	float           smooth;
	float           qasp;
