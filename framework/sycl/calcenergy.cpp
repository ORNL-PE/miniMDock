...
__inline__ __attribute__((always_inline)) float positive_power(float a, uint exp)
{
...
SYCL_EXTERNAL __inline__ __attribute__((always_inline)) float fmod_pi2(float x)
{
...
SYCL_EXTERNAL __inline__ __attribute__((always_inline)) float fast_acos(float cosine)
{
...
SYCL_EXTERNAL __inline__ __attribute__((always_inline)) sycl::float4 cross(sycl::float3 &u,
                                                                           sycl::float3 &v)
{
  sycl::float4 result;
...
__inline__ __attribute__((always_inline)) sycl::float4 cross(sycl::float4 &u, sycl::float4 &v)
{
  sycl::float4 result;
  result.x() = u.y() * v.z() - v.y() * u.z();
  result.y() = v.x() * u.z() - u.x() * v.z();
...
SYCL_EXTERNAL __inline__ __attribute__((always_inline)) sycl::float4 quaternion_multiply(sycl::float4 a,
                                                                                         sycl::float4 b)
{
  sycl::float4 result = {
...
SYCL_EXTERNAL __inline__ __attribute__((always_inline)) sycl::float4 quaternion_rotate(sycl::float4 v,
                                                                                       sycl::float4 rot)
{
  sycl::float4 result;
...
/* DPCT_ORIG 	for (uint atom_id = threadIdx.x;*/
  ...
  */
  ...
  // ================================================
/* DPCT_ORIG 	for (uint rotation_counter  = threadIdx.x;*/
...
/* DPCT_ORIG 	for (uint atom_id = threadIdx.x;*/
  ...
  // ================================================
/* DPCT_ORIG 	for (uint contributor_counter = threadIdx.x;*/
