...
#ifdef SYNCHRONOUS
/*
DPCT1010:182: SYCL uses exceptions to report errors and does not use the error
codes. The call was replaced with 0. You need to rewrite this code.
*/
/*
DPCT1009:183: SYCL uses exceptions to report errors and does not use the error
codes. The original code was commented out and a warning string was inserted.
You need to rewrite this code.
*/
/*
DPCT1003:184: Migrated API does not return error code. (*, 0) is inserted. You
may need to rewrite this code.
*/
#define LAUNCHERROR(s)                                                         \
 {                                                                             \
  /* DPCT_ORIG 		cudaError_t status = cudaGetLastError(); \*/                  \
  int status = 0;                                                              \
  /* DPCT_ORIG 		if (status != cudaSuccess) {                                  \
                          printf("Error: %s launching kernel %s\n",            \
     cudaGetErrorString(status), s); cudaDeviceReset(); exit(-1);                                                                     \
                  } \*/                                                        \
                                                                               \
  /* DPCT_ORIG 		status = cudaDeviceSynchronize(); \*/                         \
  status = (dpct::get_current_device().queues_wait_and_throw(), 0);            \
  RTERROR(status, s);                                                          \
 }
#else
#define LAUNCHERROR(s) \
...
typedef struct dpct_type_156780
{
	int             num_of_atoms;
